...
class OperationStrategy:
    ...
    @classmethod
    def concat(
        cls, operations_strategies: List["OperationStrategy"]
    ) -> "OperationStrategy":
        return OperationStrategy(
            operations=[
                x
                for operations_strategy in operations_strategies
                for x in operations_strategy.operations
            ],
            stage_delta=_assert_all_same(
                [
                    operations_strategy.stage_delta
                    for operations_strategy in operations_strategies
                ]
            ),
        )
...
def create_qwen3_moe_layer_operation_strategy(
    ...
    if role_type == RoleType.PREFILL:
        return _create_qwen3_moe_prefill_layer_operation_strategy(
            model, layer, layer_id, num_layers
        )
    elif role_type == RoleType.DECODE or role_type == RoleType.PDFUSION:
        return _create_qwen3_moe_decode_layer_operation_strategy(
            model, layer, layer_id, num_layers
        )
    else:
